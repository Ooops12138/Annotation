...
def test_hard_check_preserves_invalid_sources_and_skips_eligible_candidate() -> None:
    ...
    assert f"content-reflection-{task.task_id}-missing-teaching-material" in issue_ids
...
def test_hard_check_accepts_a_bound_explanation_and_required_material() -> None:
    task = _task()
    primary = _primary(source_refs=["src-1"], content="定义写作 $x^2$。")
    material = ContentArtifact(
        artifact_id="content-material",
    ...
    result = _content_hard_check(
        ...
        context_pack=_pack(),
        valid_source_refs={"src-1"},
        candidate_artifacts=[primary, material],
    )
